...
def arm_and_takeoffNoGPS(x, y ,x_tujuan,y_tujuan,Target):
    global step
    ...
    if x > x_tujuan + 30:
        roll = -2.5
        print("kekiri")
    elif x_tujuan + 10 < x < x_tujuan + 15:
        roll = 2.5
        print("bales kanan")
    elif x < x_tujuan - 30:
        roll = 2.5
        ...
        roll = -2.5
        print("bales kiri")
    elif x_tujuan-10 <= x <= x_tujuan+10:
         roll =0
         print("diam x")
    if y < y_tujuan - 30:
        pitch = 2.5
        print("mundur")
    elif y_tujuan - 10 > y > y_tujuan - 15:
        pitch = -2.5
        print("bales maju")
    elif y > y_tujuan + 30:
        pitch = -2.5
        ...
        pitch = 2.5
        print(" bales mundur")
    elif y_tujuan - 10 <= y <= y_tujuan+10:
        pitch = 0
        print("diam y")
    if current_altitude >= Target * 0.95:
        ...
        thrust = 0.6
    set_attitude(pitch_angle=pitch, roll_angle=roll, thrust=thrust)

def landing_disarm(x,y,x_tujuan,y_tujuan):
    global step
    thrust = 0.35
    roll = 0
    pitch = 0
...
while True:
    ...
    if  vehicle.mode == VehicleMode("GUIDED_NOGPS"):
        if step == 0:
            ...
            arm_and_takeoffNoGPS(x,y,336,270,1)
        elif step == 1:
            print("landing function")
            landing_disarm(x,y,336,270)
        else:
            break
